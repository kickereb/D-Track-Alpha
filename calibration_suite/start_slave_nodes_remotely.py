...
def start_subscriber(host):
    ...
    try:
        ...
        result = connection.run(f"cd {PATH_TO_MAIN} && sudo ./setup_node_for_calibration.sh && source camera-node/bin/activate && python slave_calibrator.py 10.0.0.14", pty=True)
